...
#if UNITY_EDITOR_WIN
            if (!System.IO.File.Exists(Application.dataPath + "/VRPanorama/StreamingAssets/ffmpeg.exe"))
            {
                GUILayout.Label("For corect functioning, VR Panorama requires FFMPEG.");
                GUILayout.Label("Download FFMPEG by going to:");
                if (GUILayout.Button("Download FFMPEG"))
                {
                    Application.OpenURL("https://www.ffmpeg.org");
                }
...
#if UNITY_EDITOR_OSX
            if (!System.IO.File.Exists(Application.dataPath + "/VRPanorama/StreamingAssets/ffmpeg"))
            {
                GUILayout.Label("For corect functioning, VR Panorama requires FFMPEG.");
                GUILayout.Label("Download FFMPEG by going to:");
                if (GUILayout.Button("Download FFMPEG"))
...
# endif
            ...
            {
                VRP.recordRT = EditorGUILayout.Toggle(new GUIContent("Record RT to video", "Captures RT rendered panorama directly to video"), VRP.recordRT);
            }
            ...
            if (VRP.captureType == VRPanorama.VRCapture.VRCaptureList.AnimationCapture)
            {
                ...
                {
                    if (GUILayout.Button("Play Panorama"))
                    {
                        PlayerPrefs.SetInt("VRPlayer", 1);
                        VRP.playPanorama = true;
                        UnityEditor.EditorApplication.isPlaying = true;
                    }
                }
